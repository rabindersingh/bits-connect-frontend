...
const AdminDashboard = () => {
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '20px' }}>
      <h1 style={{ color: '#6366F1', marginBottom: '30px', textAlign: 'center' }}>🛡️ Admin Dashboard</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '15px', marginBottom: '30px' }}>
        {Object.entries(stats).map(([key, value]) => (
          <div key={key} style={{ background: 'rgba(255, 255, 255, 0.05)', border: '2px solid #00D9FF', borderRadius: '15px', padding: '20px', textAlign: 'center' }}>
            <h3 style={{ color: '#00D9FF', fontSize: '0.9em', marginBottom: '10px', textTransform: 'capitalize' }}>{key}</h3>
            <p style={{ color: '#B3FF00', fontSize: '2.5em', fontWeight: 'bold', margin: '0' }}>{value}</p>
          </div>
        ))}
      </div>
